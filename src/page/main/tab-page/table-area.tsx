...
function getCellClassName(
  ...
  return cn(
    ...
    pinned === "right" &&
      ...
      "shadow-[-2px_0_0_0_hsl(var(--border))]",
    isHeader && "top-0 z-30 bg-sidebar",
  )
}
...
function StatusBar({
  ...
  return (
    <div className="flex-none flex items-center gap-4 px-3 h-9 border-b bg-muted/20 text-xs text-muted-foreground shrink-0">
      <span>
    ...
    () => ({
      ...
      columnPinning: {
        left: [
          ...
          ...tableUi.columnPinning.left.filter(
            (id) => id !== ROW_NUMBER_COLUMN_ID,
          ),
        ],
        right: tableUi.columnPinning.right,
  ...
  return (
    <div className="size-full flex flex-col overflow-hidden">
      ...
      {result.columns.length === 0 ? (
        ...
        <>
          <div className="flex-none flex items-center gap-1 px-2 h-9 border-b bg-background/90 shrink-0">
            <ColumnVisibilityMenu
              table={table}
              dataColumnCount={result.columns.length}
            />
            <CopyMenu table={table} activeCell={tableUi.activeCell} />
            <ExportMenu table={table} defaultName={exportName} />
            <div className="ml-auto text-[10px] text-muted-foreground/70">
              当前排序: {getSortingSummary(table)}
            </div>
          </div>
